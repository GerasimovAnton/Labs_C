...
int main() {
    ...
    printf("This program takes a string, divides it into words, puts them in a tree and prints\nINPUT STRING = ");
     while ((ch = getchar()) != '\n') {
        str[i++] = ch;
    }
    ...
    {
        if((str[i] == ' ' || str[i] == '\0') && isStr == 1)
        {
            isStr = 0;
            indexs[indx++] = i;
            if( str[i] == '\0') break;
        } 
            else if(str[i] != ' ' && isStr == 1)
        {
            isStr = 1;
        } 
            else if (str[i] != ' ' && isStr == 0)
        {
            isStr = 1;
            indexs[indx++] = i;
        }
    ...
    for (size_t i = 0; i < indx; i+=2)
    {
        int s = indexs[i];
        int f = indexs[i+1];
        
        int p = 0;
        for (size_t m = s; m < f; m++)
        words[wnum][p++] = str[m];
    ...
    {
        set_tree(tree, decimal(i), string(words[i]));
    }
